...
INPUTS = ['g100', 'g101', 'g102', 'g103', 'g104', 'g109', 'g1170', 'g1173', 'g1176', 'g1179',
...
WIRES = ['g10377', 'g10379', 'g10455', 'g10457', 'g10459', 'g10461', 'g10463', 'g10465',
         ...
         'test_so6', 'test_so7', 'test_so8', 'test_so9', 'test_so10', 'N599', 'g1289', 'g1882', 'g312', 'g452', 'g11257', 'g123', 'g207', 'g713', 'g1153',
         ...
         'Tg1', 'Tg1_OUT1', 'Tg1_OUT1234', 'Tg1_OUT2', 'Tg1_OUT3', 'Tg1_OUT4', 'Tg1_OUT5', 'Tg1_OUT5678',
         'Tg1_OUT6', 'Tg1_OUT7', 'Tg1_OUT8', 'Tg1_Trigger1', 'Tg2',
         'Tg2_OUT1', 'Tg2_OUT1234', 'Tg2_OUT2', 'Tg2_OUT3', 'Tg2_OUT4', 'Tg2_OUT5', 'Tg2_OUT5678',
         'Tg2_OUT6', 'Tg2_OUT7', 'Tg2_OUT8', 'Tg2_Trigger2',  'Trigger_select']


def getControlOLD(filename: str):
    df = pd.read_table("vectorfiles/data_out.txt", delimiter='\t', dtype='string', header=None)
    # print(df.memory_usage(index=True).sum())
    print(df.shape)

    control_df = pd.DataFrame(np.zeros((NUM_WIRES, NUM_INPUTS)), index=WIRES, columns=INPUTS)
    control = 0

    for i, inwire in enumerate(INPUTS):
        print("CONTROL VALUE FOR INPUT NUMBER " + str(i))
        for netwire in range(NUM_WIRES):
            for index in range(0, len(df), 2):
                if df.loc[index, netwire + NUM_INPUTS] != df.loc[index + 1, netwire + NUM_INPUTS]:
                    control += 1
            control_df.loc[netwire, inwire] = control / NUM_SAMPLES
            control = 0

    control_df.to_csv("s15850.csv")
...
def getControl(infile: str, outfile: str):
    # control_df = pd.DataFrame(np.zeros((len(WIRES), len(INPUTS))), index=WIRES, columns=INPUTS)
    tt = open(infile)
    control_vals = []
    for i, inwire in enumerate(INPUTS):
        print("CONTROL VALUE FOR INPUT NUMBER " + str(i))
        ...
        for index in range(0, NUM_SAMPLES):
            ...
            line0_val, line1_val = removex(line0, line1)
            # xor = line0_val ^ line1_val
            xor = bin(line0_val ^ line1_val)[2:].zfill(len(WIRES))
            xor = [int(k) for k in xor] # [0, 1, 0, 0, 0, 0]
...
def removex(line0: str, line1: str):
    while 'x' in line0:
        loc = line0.find('x')
        if line1[loc] == 'x':
            line0 = line0[:loc] + '0' + line0[loc+1:]
...
def heuristic(inCSV: str, threshold: float = 0.01):
    control_heuristic = []
    ...
    heuristic_df.to_csv("test_fixed.csv")
    


if __name__ == "__main__":
    # print(len(INPUTS) + len(WIRES))
    # getControl("data_out_s15850.txt", "control_s15850.csv")
    heuristic("control_s15850_fixed.csv")

    # print(INPUTS.index('test_se'))
    # print(len(WIRES) + 88)
